...
import { Plus, Search, Loader2, AlertCircle, Star } from 'lucide-react'
...
function calculerMarge(r: Recette): { brut: number; pct: number } {
  const cout = Number(r.cout_matieres_forfait || 0) + Number(r.cout_emballage || 0)
...
export default function Recettes() {
  ...
  const [categorieFiltre, setCategorieFiltre] = useState<CategorieRecette | 'toutes'>(
    'toutes'
  )
  ...
  return (
    <div>
      <header className="flex items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="font-serif text-3xl sm:text-4xl tracking-tight">Recettes</h1>
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_auto] mb-6">
        ...
        <select
          ...
          onChange={(e) =>
            setCategorieFiltre(e.target.value as CategorieRecette | 'toutes')
          }
          ...
          {recettesFiltrees.map((r) => {
            ...
            return (
              <li key={r.id}>
                <Link
                  to={`/recettes/${r.id}`}
                  className="card block hover:shadow-md transition focus:outline-none focus:ring-2 focus:ring-dusty-pink/50"
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="min-w-0">
                      <h3 className="font-serif text-xl text-warm-brown truncate flex items-center gap-2">
                        {r.favori && (
                          <Star
                            className="h-4 w-4 fill-caramel text-caramel flex-shrink-0"
                            aria-label="Favori"
                          />
                        )}
                        {r.nom}
                    ...
                    {!r.actif && (
                      <span className="rounded-full bg-soft-taupe/40 text-warm-brown/70 text-xs px-2 py-0.5">
                        Inactif
                      </span>
                    )}
                  </div>
